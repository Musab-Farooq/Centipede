...
void drawmushroom(sf::RenderWindow& window, float mushroom[][3], sf::Sprite mushroomSprite[]);
void bulletmushroomcollision(sf::RenderWindow& window, float bullet[], float mushroom[][3], sf::Sprite mushroomSprite[], int& score);
void drawcentipede(sf::RenderWindow& window, float centipede[][3], sf::Sprite centipedeSprite[], sf::Sprite& headSprite);
void resetbullet(float bullet[], float player[]);
void movecentipede(float centipede[][3], sf::Clock& centipedeClock, float dirction[], float mushroom[][3]);
void centipedebulletcollision(sf::RenderWindow& window, float centipede[][3], float bullet[], float dirction[], sf::Sprite centipedeSprite[], sf::Texture& headTexture, float mushroom[][3], sf::Sprite mushroomSprite[], int& score);
...
void drawscorpion(sf::RenderWindow& window, float scorpion[][3], sf::Sprite scorpionSprite[]);
...
int main()
{
    ...
    float dirction[12] = {}; // direction variable for change in direction of centipede when it hits the wall or mushroom
    for (int i = 0; i < 12; i++)
    {
        dirction[i] = -32;
    }
    ...
    for (int i = 0; i < 1; i++)
    {
        ...
        scorpionSprite[i].setTextureRect(sf::IntRect(96, 0, boxPixelsX, boxPixelsY));
    }

    while (window.isOpen())
    {
        //*************** Function calling area. Here all the created functions are called ****************
        ...
        centipedebulletcollision(window, centipede, bullet, dirction, centipedeSprite, headTexture, mushroom, mushroomSprite, score);
        ...
        sf::Time elapsed = scorpionRespawnClock.getElapsedTime(); // this is function for repeatitive regeneration of scorpion
        ...
        bool allSegmentsDead = true; // this is the function for generating centipede again and again
        for (int i = 0; i < 12; i++)
        {
        ...
        while (window.pollEvent(e)) {
            ...
            else if (e.type == sf::Event::KeyPressed) // keyprssed functions
            {
                ...
                if (e.key.code == sf::Keyboard::Up && player[y] > 800 && player[y] < 960)
                {
                    player[y] -= boxPixelsY;
                }
...
void bulletmushroomcollision(sf::RenderWindow& window, float bullet[], float mushroom[][3], sf::Sprite mushroomSprite[], int& score)
{
    for (int i = 0; i < 25; i++)
    {
        if (mushroom[i][exists] && bullet[exists] && bullet[x] >= mushroom[i][x] && bullet[x] <= mushroom[i][x] + boxPixelsX && bullet[y] >= mushroom[i][y] && bullet[y] <= mushroom[i][y] + boxPixelsY)
        {
            bullet[exists] = false;
            mushroom[i][exists]++;
            if (mushroom[i][exists] >= 2)
            {
                if (mushroom[i][exists] == 2)
                {
                    mushroomSprite[i].setTextureRect(sf::IntRect(96, 0, boxPixelsX, boxPixelsY));
                    window.draw(mushroomSprite[i]);
                }
            }
...
void movecentipede(float centipede[][3], sf::Clock& centipedeClock, float dirction[], float mushroom[][3])
{
    if (centipedeClock.getElapsedTime().asMilliseconds() < 40)
        return;
    ...
    for (int i = 0; i < 12; i++)
    {
        ...
        for (int j = 0; j < 25; j++)
        {
            if (centipede[i][exists] && mushroom[j][exists] && centipede[i][x] >= mushroom[j][x] && centipede[i][x] <= mushroom[j][x] + boxPixelsX && centipede[i][y] >= mushroom[j][y] && centipede[i][y] <= mushroom[j][y] + boxPixelsY)
            {
                centipede[i][y] += 32;
                dirction[i] = -dirction[i];
            }
        }
        if ((centipede[i][y] == 960) && (centipede[i][x] < 0 || centipede[i][x] > 928))
        {
            centipede[i][y] -= 160;
        }
    }
...
void centipedebulletcollision(sf::RenderWindow& window, float centipede[][3], float bullet[], float dirction[], sf::Sprite centipedeSprite[], sf::Texture& headTexture, float mushroom[][3], sf::Sprite mushroomSprite[], int& score)
{
    for (int i = 0; i < 12; i++)
    {
        if (centipede[i][exists] && bullet[exists] && bullet[x] >= centipede[i][x] && bullet[x] <= centipede[i][x] + boxPixelsX && bullet[y] >= centipede[i][y] && bullet[y] <= centipede[i][y] + boxPixelsY)
        {
            int mushroomIndex = -1;
            for (int j = 0; j < 25; j++)
            {
            ...
            if (mushroomIndex != -1)
            {
                ...
                mushroomSprite[mushroomIndex].setTextureRect(sf::IntRect(0, 0, boxPixelsX, boxPixelsY));
            }
            if (centipede[i][y] <= 960 && centipede[i][y] >= 800)
            {
                if (mushroomIndex != -1)
                {
                    ...
                    mushroom[mushroomIndex][y] = centipede[i][y];
                    mushroom[mushroomIndex][exists] = 1;
                    mushroomSprite[mushroomIndex].setTextureRect(sf::IntRect(0, 32, boxPixelsX, boxPixelsY));
                }
            }
            ...
            if (centipede[i + 1][exists])
            {
                ...
                centipedeSprite[i + 1].setTextureRect(sf::IntRect(0, 0, boxPixelsX, boxPixelsY));
            }
...
void centipedeplayercollision(float centipede[][3], float player[])
{
    for (int i = 0; i < 12; i++)
    {
        if (centipede[i][exists] && player[exists] && player[x] >= centipede[i][x] && player[x] <= centipede[i][x] + boxPixelsX && player[y] >= centipede[i][y] && player[y] <= centipede[i][y] + boxPixelsY)
        {
...
void playermushroomcollision(float mushroom[][3], float player[])
{
    for (int i = 0; i < 25; i++)
    {
        if (mushroom[i][exists] && player[exists] && player[x] >= mushroom[i][x] && player[x] <= mushroom[i][x] + boxPixelsX && player[y] >= mushroom[i][y] && player[y] <= mushroom[i][y] + boxPixelsY)
        {
            const float deathVolume = 50.0f;
            sf::SoundBuffer deathSoundBuffer;
            sf::Sound deathSound;
...
void movescorpion(float scorpion[][3], sf::Clock& scorpionClock)
{
    ...
    for (int i = 0; i < 1; i++)
    {
        if (scorpion[i][exists])
        {
            ...
            if (scorpion[i][x] < 0 || scorpion[i][y] < 0 || scorpion[i][y] > resolutionY)
            {
...
void scorpioncollision(float bullet[], float scorpion[][3], int& score)
{
    for (int i = 0; i < 1; i++)
    {
        if (scorpion[i][exists] && bullet[exists] && bullet[x] >= scorpion[i][x] && bullet[x] <= scorpion[i][x] + boxPixelsX && bullet[y] >= scorpion[i][y] && bullet[y] <= scorpion[i][y] + boxPixelsY)
        {
            for (int i = 0; i < 1; i++)
            {
...
void playerscorpioncollision(float scorpion[][3], float player[])
{
    for (int i = 0; i < 1; i++)
    {
        if (scorpion[i][exists] && player[exists] && player[x] >= scorpion[i][x] && player[x] <= scorpion[i][x] + boxPixelsX && player[y] >= scorpion[i][y] && player[y] <= scorpion[i][y] + boxPixelsY)
        {
            const float deathVolume = 50.0f;
            sf::SoundBuffer deathSoundBuffer;
            sf::Sound deathSound;
